...
void OrderBeverageServiceHandler::PlaceOrder(std::string& _return, const int64_t city){
    ...
    // fill in a default
    std::string _beverage = "lemonade";

    // 4. call the remote procedure : GetBeverage
    try {
	  if(weatherType == WeatherType::type::WARM)
		  beverage_preference_client->GetBeverage(_beverage, BeverageType::type::COLD);
	  else
		  beverage_preference_client->GetBeverage(_beverage, BeverageType::type::HOT);
    } catch (...) {
